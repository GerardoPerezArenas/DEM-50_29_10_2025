...
public class MeLanbide11DAO {
    ...
    // ============================= MÉTODOS RSB DESGLOSE =============================

    /**
     * Obtiene el desglose RSB filtrado por número de expediente y ID de contratación
     * @param numExpediente Número de expediente
     * @param idContratacion ID de la contratación
     * @param con Conexión a la base de datos
     * @return Lista de complementos RSB
     * @throws Exception
     */
    public List<DesgloseRSBVO> getDesgloseRSB(String numExpediente, Integer idContratacion, Connection con) throws Exception {
        if (numExpediente == null || numExpediente.trim().isEmpty()) {
            return new ArrayList<DesgloseRSBVO>();
        }
        ...
        try {
            ...
            String sql = "SELECT * FROM " + tabla + " WHERE TRIM(NUM_EXP) = ?";
            
            // Si se proporciona ID de contratación, filtrar por DNI de esa contratación
            if (idContratacion != null) {
                String dniContratacion = getDniContratacionById(numExpediente, idContratacion.toString(), con);
                if (dniContratacion != null && !dniContratacion.trim().isEmpty()) {
                    sql += " AND UPPER(REPLACE(REPLACE(TRIM(DNICONTRSB),' ',''),'-','')) = UPPER(REPLACE(REPLACE(TRIM(?),' ',''),'-',''))";
                }
            }
            ...
            ps = con.prepareStatement(sql);
            ps.setString(1, numExpediente.trim());
            if (idContratacion != null) {
                String dniContratacion = getDniContratacionById(numExpediente, idContratacion.toString(), con);
                if (dniContratacion != null && !dniContratacion.trim().isEmpty()) {
                    ps.setString(2, dniContratacion.trim());
                }
            }
            
            rs = ps.executeQuery();
            while (rs.next()) {
                DesgloseRSBVO desglose = (DesgloseRSBVO) MeLanbide11MappingUtils.getInstance().mapRSB(rs, DesgloseRSBVO.class);
                lista.add(desglose);
            }
            
        } catch (Exception ex) {
            log.error("Error recuperando desglose RSB", ex);
            throw new Exception(ex);
        } finally {
    ...
    /**
     * Inserta un nuevo complemento RSB
     * @param complemento Complemento a insertar
     * @param con Conexión a la base de datos
     * @return true si se insertó correctamente
     * @throws Exception
     */
    public boolean insertarComplementoRSB(DesgloseRSBVO complemento, Connection con) throws Exception {
        if (complemento == null) {
            return false;
        }
        ...
        try {
            ...
            // Generar nuevo ID
            int nuevoId = recogerIDInsertar("MELANBIDE11_DESGRSB_SEQ", con);
            
            String sql = "INSERT INTO " + tabla + " (ID, NUM_EXP, DNICONTRSB, RSBTIPO, RSBCONCEPTO, " +
                        "RSBPERIODICIDAD, RSBIMPORTE, RSBPRORRATEADO, RSBCOMPUTABLE, RSBORDEN) " +
                        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            
            ps = con.prepareStatement(sql);
            ps.setInt(1, nuevoId);
            ps.setString(2, complemento.getNumExp());
            ps.setString(3, complemento.getDniContRSB());
            ps.setString(4, complemento.getTipoConcepto());
            ps.setString(5, complemento.getConcepto());
            ps.setString(6, complemento.getPeriodicidad());
            ps.setDouble(7, complemento.getImporte() != null ? complemento.getImporte() : 0.0);
            ps.setDouble(8, complemento.getProrrateado() != null ? complemento.getProrrateado() : 0.0);
            ps.setDouble(9, complemento.getComputable() != null ? complemento.getComputable() : 0.0);
            ps.setInt(10, complemento.getOrden() != null ? complemento.getOrden() : 1);
            
            int rowsAffected = ps.executeUpdate();
            return rowsAffected > 0;
            
        } catch (Exception ex) {
            log.error("Error insertando complemento RSB", ex);
            throw new Exception(ex);
        } finally {
            if (ps != null) try { ps.close(); } catch (Exception ignore) {}
        }
    ...
    /**
     * Actualiza un complemento RSB existente
     * @param complemento Complemento a actualizar
     * @param con Conexión a la base de datos
     * @return true si se actualizó correctamente
     * @throws Exception
     */
    public boolean actualizarComplementoRSB(DesgloseRSBVO complemento, Connection con) throws Exception {
        ...
        try {
            ...
            String sql = "UPDATE " + tabla + " SET " +
                        "RSBTIPO = ?, RSBCONCEPTO = ?, RSBPERIODICIDAD = ?, " +
                        "RSBIMPORTE = ?, RSBPRORRATEADO = ?, RSBCOMPUTABLE = ?, RSBORDEN = ? " +
                        "WHERE ID = ?";
            
            ps = con.prepareStatement(sql);
            ps.setString(1, complemento.getTipoConcepto());
            ps.setString(2, complemento.getConcepto());
            ps.setString(3, complemento.getPeriodicidad());
            ps.setDouble(4, complemento.getImporte() != null ? complemento.getImporte() : 0.0);
            ps.setDouble(5, complemento.getProrrateado() != null ? complemento.getProrrateado() : 0.0);
            ps.setDouble(6, complemento.getComputable() != null ? complemento.getComputable() : 0.0);
            ps.setInt(7, complemento.getOrden() != null ? complemento.getOrden() : 1);
            ps.setInt(8, complemento.getId());
            
            int rowsAffected = ps.executeUpdate();
            return rowsAffected > 0;
            
        } catch (Exception ex) {
            log.error("Error actualizando complemento RSB", ex);
            throw new Exception(ex);
        } finally {
            if (ps != null) try { ps.close(); } catch (Exception ignore) {}
        }
    ...
    /**
     * Elimina un complemento RSB por ID
     * @param idComplemento ID del complemento a eliminar
     * @param con Conexión a la base de datos
     * @return true si se eliminó correctamente
     * @throws Exception
     */
    public boolean eliminarComplementoRSB(Integer idComplemento, Connection con) throws Exception {
        ...
        try {
            ...
            log.error("Error eliminando complemento RSB", ex);
            throw new Exception(ex);
        } finally {
            if (ps != null) try { ps.close(); } catch (Exception ignore) {}
        }
    ...
    /**
     * Obtiene los tipos de complementos disponibles
     * @param con Conexión a la base de datos
     * @return Lista de tipos de complementos
     * @throws Exception
     */
    public List<ComplementosPorTipo> getTiposComplementos(Connection con) throws Exception {
        List<ComplementosPorTipo> tipos = new ArrayList<ComplementosPorTipo>();
        
        // Por simplicidad, devolvemos tipos estáticos básicos
        // En un entorno real, esto vendría de una tabla de configuración
        tipos.add(new ComplementosPorTipo(1.0, 0.0)); // Salariales
        tipos.add(new ComplementosPorTipo(0.0, 1.0)); // Extrasalariales
